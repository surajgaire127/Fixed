...
std::string get_Key_From_Public(std::string publicKey) {
    // Ensure the publicKey is long enough
    if (publicKey.length() < 98) return "000000000";

    // Build the 9-character key safely
    std::string keyDes = "";
    keyDes += publicKey[57];  // 58th char
    keyDes += publicKey[61];  // 62nd char
    keyDes += publicKey[9];   // 10th char
    keyDes += publicKey[46];  // 47th char
    keyDes += publicKey[19];  // 20th char
    keyDes += publicKey[32];  // 33rd char
    keyDes += publicKey[86];  // 87th char
    keyDes += publicKey[97];  // 98th char
    keyDes += publicKey[13];  // 14th char
    return keyDes;
}
